...
with open('train_binary.jsonl', 'r', encoding='utf-8') as f:
    raw_content = f.read()
...
# Extract unique names only to build the reference mapping ids
unique_functions = sorted(list(set(item['id'] for item in modified_data)))

# Create the group mapping dictionary (Name -> Integer ID)
group_mapping = {func_name: idx for idx, func_name in enumerate(unique_functions)}

# Process the dataset into parallel text and label arrays (duplicates are preserved)
binary_codes = []
labels = []
